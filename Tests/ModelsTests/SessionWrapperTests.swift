...
struct SessionWrapperTests {
  ...
  func parseValidTimeRangeDay1() {
    ...
    if let dateInterval = wrapper.dateInterval {
      ...
      let startComponents = calendar.dateComponents([.year, .month, .day, .hour, .minute], from: dateInterval.start)
      let endComponents = calendar.dateComponents([.year, .month, .day, .hour, .minute], from: dateInterval.end)
